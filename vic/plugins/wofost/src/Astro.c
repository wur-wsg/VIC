...
void Astro(SimUnit *Grid)
{
   ...
   /* Astronomical day length */
    ...
    if (AOB <= 1.0)
    {   
        DSinB  = 3600.*(Grid->met->Daylength*Grid->met->SinLD+(24./CONST_PI)*Grid->met->CosLD*sqrt(1.-AOB*AOB));
        Grid->met->DSinBE = 3600.*(Grid->met->Daylength*(Grid->met->SinLD+0.4*(Grid->met->SinLD*Grid->met->SinLD + Grid->met->CosLD*Grid->met->CosLD*0.5))+
            12.*Grid->met->CosLD*(2.+3.*0.4*Grid->met->SinLD)*sqrt(1.-AOB*AOB)/CONST_PI);
    }  
    else
    {
    ...
    if (Grid->met->AtmosphTransm <= 0.35 && Grid->met->AtmosphTransm > 0.07) 
       FractionDiffuseRad = 1.-2.3*pow((Grid->met->AtmosphTransm-0.07), 2.);
  
    if (Grid->met->AtmosphTransm < 0.07)  
       FractionDiffuseRad = 1.0;
    
    Grid->met->DiffRadPP = 0.5 * FractionDiffuseRad * Grid->met->AtmosphTransm * SolarConstant;
}
