...
float DyingLeaves(SimUnit *Grid)
{
    float tiny = 0.001;
    ...
    Death += Grid->crp->st.leaves * Grid->crp->prm.DyingLeaves_NPK_Stress * (1. - Grid->crp->NPK_Indx);
    ...
    if (Grid->crp->LeaveProperties != NULL)
    {
        Grid->crp->LeaveProperties->weight = Grid->crp->LeaveProperties->weight - Death;

        while(Grid->crp->LeaveProperties != NULL && Grid->crp->LeaveProperties->age > Grid->crp->prm.LifeSpan)
        {
            wipe                   = Grid->crp->LeaveProperties; 
