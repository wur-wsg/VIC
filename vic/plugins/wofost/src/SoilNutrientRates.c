...
void SoilNutrientRates(SimUnit *Grid)
{
    ...
    if (Grid->crp->st.Development > 0. && Grid->crp->st.Development <= Grid->crp->prm.DevelopmentStageNLimit)
    {   /* NPK rates that come available through mineralization, cannot exceed */
        /* the available NPK for mineralization                                */
        Grid->ste->rt_N_mins = min(Grid->mng->N_Mins * Grid->mng->NRecoveryFrac, Grid->ste->st_N_mins); 
        Grid->ste->rt_P_mins = min(Grid->mng->P_Mins * Grid->mng->PRecoveryFrac, Grid->ste->st_P_mins); 
        Grid->ste->rt_K_mins = min(Grid->mng->K_Mins * Grid->mng->KRecoveryFrac, Grid->ste->st_K_mins); 
    }
    else
    {
