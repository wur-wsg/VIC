...
void Partioning(SimUnit *Grid)
{
    ...
    if (Grid->soil->WaterStress < Grid->crp->N_st.Indx)
    {
        ...
        Grid->crp->fac_so = Afgen(Grid->crp->prm.Storage, &(Grid->crp->st.Development));
    }
    else
    {
        flv = Afgen(Grid->crp->prm.Leaves, &(Grid->crp->st.Development));
        factor = exp(-Grid->crp->prm.N_lv_partitioning * ( 1. - Grid->crp->N_st.Indx));
        Grid->crp->fac_lv = flv * factor;
        Grid->crp->fac_ro = Afgen(Grid->crp->prm.Roots, &(Grid->crp->st.Development));
        Grid->crp->fac_st = Afgen(Grid->crp->prm.Stems, &(Grid->crp->st.Development)) + flv - Grid->crp->fac_lv;
        Grid->crp->fac_so = Afgen(Grid->crp->prm.Storage, &(Grid->crp->st.Development));
    }
}	
