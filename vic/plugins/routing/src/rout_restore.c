...
rout_restore(void)
{
    ...
    // in the config) are both enabled. Note: rout_restore() is only reached
    // when ROUTING=TRUE (guarded in plugin_restore() in plugin_setup.c).
    //
    // When WATERUSE or NONRENEW_WITH is off (e.g. a natural run), zero-initialize
    // silently -- no NC lookup needed, no spurious warning.
    //
    ...
    if (plugin_options.WATERUSE && plugin_options.NONRENEW_WITH) {
        ...
        // WATERUSE or NONRENEW_WITH disabled: deficit not tracked, zero silently.
        for (i = 0; i < local_domain.ncells_active; i++) {
            rout_var[i].nonrenew_deficit = 0.0;
        }
    }
