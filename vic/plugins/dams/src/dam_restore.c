...
dam_check_init_state_file(void)
{
    ...
    if (mpi_rank == VIC_MPI_ROOT) {
        ...
        get_nc_field_int(&(filenames.init_state), "dam_class", d1start, d1count,
                         state_dam_classes);
        ...
        if (!dmy_equal(&start_dmy, &init_dmy)) {
            log_err(
                "Cannot restore dams when the INIT_STATE time is not equal to simulation start time.");
        }
    }
}
    ...
    for (j = 0; j < plugin_options.NDAMTYPES; j++) {
        for (i = 0; i < local_domain.ncells_active; i++) {
            ...
            if (dam_index != NODATA_DAM) {
                ...
                get_nc_field_double(&(filenames.init_state),
                                    state_metadata[N_STATE_VARS +
                                                   STATE_DAM_HISTORY_DEMAND].varname,
                ...
                get_nc_field_double(&(filenames.init_state),
                                    state_metadata[N_STATE_VARS +
                                                   STATE_DAM_HISTORY_INFLOW].varname,
                                    d2start, d2histcount, d2histvar);
                for (k = 0; k < MONTHS_PER_YEAR * DAM_HIST_YEARS; k++) {
