                    ...
                    soil_con_struct  soil_con)
{
    ...
    if (ErrFlag == ERROR || ErrFlag > 0) {
        log_err("Error calculating volume: depth %f, maxdepth = %f, "
                "volume %e, maxvolume %e",
                lake_con->maxdepth, lake_con->maxdepth, 
                lake_con->maxvolume, lake_con->maxvolume);
    }

    // compute volume corresponding to mindepth
    ErrFlag = get_volume(lake_con, lake_con->mindepth, &(lake_con->minvolume));
    if (ErrFlag == ERROR || ErrFlag > 0) {
        log_err("Error calculating depth: depth %f, maxdepth = %f, "
                "volume %e, maxvolume %e",
                lake_con->mindepth, lake_con->maxdepth,
                lake_con->minvolume, lake_con->maxvolume);
    }
}
